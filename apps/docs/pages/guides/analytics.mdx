...
The Trail Analytics dashboard in the Pro Editor ([/dashboard/analytics](https://app.gettrailguide.com/dashboard/analytics)) has three levels of detail:

**L1 — Snapshot bar**
Four at-a-glance metrics, each with a trend arrow comparing the current period to the previous one:
...
**L3 — Raw sessions table** (collapsed by default)
Every individual session with outcome badge (Completed / Dropped / In Progress), steps viewed, exit step, and relative timestamp.
